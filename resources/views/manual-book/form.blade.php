        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">Manual Book</h4>
                </div>
                <div class="card-body">
                    <form class="form form-vertical" method="POST" action="{{@$value != null ?  url('manual-book/'.@$value->id) : url('manual-book')}}">
                        {{csrf_field()}}
                        @if(@$value)
                            {{method_field('patch')}}
                        @endif
                        <div class="row">
                            <div class="col-12">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="name" class="form-control" name="name" placeholder="Nama" value="{{@$value->name}}" required>
                                </div>

                                <div class="form-group">
                                    <label for="name">URL</label>
                                    <input type="text" id="url" class="form-control" name="url" placeholder="URL" value="{{@$value->url}}" required>
                                </div>

                                <div class="form-group">
                                    <label for="name">Deskripsi</label>
                                    <textarea id="description" class="form-control" name="description" placeholder="Deskripsi" required>{{@$value->description}}</textarea>
                                </div>
                            </div>
