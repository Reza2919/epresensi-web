        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">{{ @$libur ? 'Ubah' : 'Tambah' }} Libur</h4>
                </div>
                <div class="card-body">
                    <form class="form form-vertical" method="POST" action="{{ @$libur ? route('libur.update',[$libur->id_libur]) : route('libur.store') }}">
                        @csrf
                        @if (@$libur)
                            @method('patch')
                        @endif
