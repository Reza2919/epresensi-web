@extends('layouts.app')
@section('title','User - Presensi KEMNAKER')
@push('css')
<!-- BEGIN: Vendor CSS-->
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">{{ @$u ? 'Ubah' : 'Tambah' }} User</h4>
                </div>
                <div class="card-body">
                    <form class="form form-vertical" method="POST" action="{{ @$u ? route('user.update',[$u->id_user]) : route('user.store') }}">
                        @csrf
                        @if (@$u)
                            @method('patch')
                        @endif
                        <div class="row">
                            <div class="col-12">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="name" class="form-control" name="name" placeholder="Nama User" value="{{ @$u->name }}" required>
                                </div>
                            </div>
                            <div class="col-12">
                                <div class="form-group">
                                    <label for="email">Email</label>
                                    <input type="email" id="email" class="form-control" name="email" placeholder="Email" value="{{ @$u->email }}" required>
                                </div>
                            </div>
                            <div class="col-12">
                                <div class="form-group">
